...
def resolve_data_file() -> Path:
    """Resolve the workbook path, preferring the new VSquad env var.

    IBD_INVENTORY_FILE is kept for backward compatibility.
    """
    env_path = os.getenv("VSQUAD_INVENTORY_FILE") or os.getenv("IBD_INVENTORY_FILE")
    if env_path:
        return Path(env_path)
    return DEFAULT_DATA_FILE


def safe_metric_value(value, default: str = "—"):
    """Convert pandas missing/scalar values to a Streamlit-safe metric value.

    st.metric does not accept pandas.NA, so every study-level value displayed
    as a metric should pass through this helper.
    """
    if value is None:
        return default
    try:
    ...
    # Convert NumPy/pandas scalar objects to native Python values where possible.
...
def load_data_or_stop() -> dict[str, pd.DataFrame]:
    ...
    try:
        ...
        st.caption("Set VSQUAD_INVENTORY_FILE (or the legacy IBD_INVENTORY_FILE) to a valid .xlsx path, or place the workbook in the project data folder.")
        st.stop()
    return data
...
def page_overview(data: dict[str, pd.DataFrame], study_ids: set):
    ...
    # Normalize all high-level availability fields so blank cells are shown
    # consistently as Unknown rather than None/<NA>.
    non_status_cols = {
        "Study ID",
        "Study Name",
    ...
    for col in [c for c in avail.columns if c not in non_status_cols]:
...
def page_external_analysis(data: dict[str, pd.DataFrame]):
    """Explore external datasets/sources registered in 09_EXTERNAL_ANALYSIS.

    This table is portfolio-level and is intentionally independent of the
    clinical-study filters in the sidebar because an external dataset may not
    map one-to-one to an internal Study ID.
    """
    df = data.get("external_analysis", pd.DataFrame()).copy()

    section(
        "External analysis",
        "Track external datasets and sources, access conditions, internal locations and how they are being used by VSquad.",
    )

    if df.empty:
        st.info("No external analysis records are currently registered in 09_EXTERNAL_ANALYSIS.")
        return

    # Normalize access status only for presentation/metrics while preserving
    # explicit business states such as Restricted.
    if "Access Status" in df.columns:
        df["Access Status"] = (
            df["Access Status"]
            .fillna("Unknown")
            .astype(str)
    ...
    with c1:
        domains = st.multiselect(
            "Disease / domain",
            safe_sorted(df.get("Disease / Domain", [])),
            key="ext_domain",
        )
    with c2:
        modalities = st.multiselect(
            "Data modality",
            safe_sorted(df.get("Data Modality", [])),
            key="ext_modality",
        )
    with c3:
        source_types = st.multiselect(
            "Source type",
            safe_sorted(df.get("Source Type", [])),
            key="ext_source_type",
        )
    with c4:
        access_statuses = st.multiselect(
            "Access status",
            safe_sorted(df.get("Access Status", [])),
            key="ext_access_status",
        )

    if domains and "Disease / Domain" in df.columns:
        df = df[df["Disease / Domain"].isin(domains)]
    if modalities and "Data Modality" in df.columns:
        df = df[df["Data Modality"].isin(modalities)]
    if source_types and "Source Type" in df.columns:
        df = df[df["Source Type"].isin(source_types)]
    if access_statuses and "Access Status" in df.columns:
        df = df[df["Access Status"].isin(access_statuses)]

    available = 0
    pending = 0
    restricted = 0
    if "Access Status" in df.columns:
        status_lower = df["Access Status"].astype(str).str.strip().str.lower()
        available = int((status_lower == "available").sum())
        pending = int((status_lower == "pending").sum())
        restricted = int((status_lower == "restricted").sum())

    providers = (
        int(df["Provider / Organization"].dropna().nunique())
        if "Provider / Organization" in df.columns
        else 0
    )

    kpi_row([
        ("External datasets", len(df), "External datasets/sources matching the page filters"),
        ("Available", available, "External datasets currently marked Available"),
        ("Pending", pending, "External datasets currently marked Pending"),
        ("Restricted", restricted, "External datasets currently marked Restricted"),
        ("Providers", providers, "Distinct external providers/organizations"),
    ])
    ...
    with c1:
        if "Access Status" in df.columns:
            status_distribution_chart(df, "Access Status", "External data access status")
    with c2:
        if "Data Modality" in df.columns and not df.empty:
            modality_counts = (
                df["Data Modality"]
                .fillna("Unknown")
                .astype(str)
                .value_counts()
                .rename_axis("Data Modality")
            ...
            donut_chart(
                ...
                "Data Modality",
                "Datasets",
                "External data by modality",
            )

    section(
        "External dataset inventory",
        "Use the links, when available, to navigate directly to the external repository, license/terms or internal data location.",
    )
    show_table(
        df,
        height=520,
    ...
    download_csv(df, "external_analysis_filtered.csv")
...
def study_detail_panel(data: dict[str, pd.DataFrame], study_ids: set):
    ...
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Study ID", safe_metric_value(sid))
    c2.metric("Disease", safe_metric_value(row.get("Disease")))
    c3.metric("Patients", safe_metric_value(row.get("Patients")))
    c4.metric("Videos", safe_metric_value(row.get("Videos")))
...
def main():
    ...
    page = st.sidebar.radio(
        "Navigation",
        ["Overview", "Data Availability", "Assets", "Processing", "Data Splits", "ARD & Variables", "Analysis & Traceability", "External Analysis"],
    )
    st.sidebar.markdown("---")
    ...
    elif page == "External Analysis":
        page_external_analysis(data)
