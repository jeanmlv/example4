...
def _render_variable_explorer(filtered: dict[str, pd.DataFrame]) -> None:
    ...
    st.caption("Explore ARD PARAMCD variables overall and across visits. AVAL is treated as numeric and AVALC as categorical.")

    files = discover_ard_files()
    if not files:
        st.info(
            "No ARD CSV was found. Create an `ard_data` folder beside app.py and place one or more ARD CSV files there "
            "(or use `data/ard`). The inventory workbook can remain where it is."
        )
        return
    ...
    eligible_files = filter_ard_files_for_studies(files, study_ids)

    # If a filename contains a Study ID, show it as part of the friendly label.
    all_known_ids = study_ids
    file_labels = {}
    for path in eligible_files:
        sid = study_id_from_filename(path, all_known_ids)
    ...
    if catalog.empty:
        st.warning("No columns matching PARAMCD_<parameter>_AVAL or PARAMCD_<parameter>_AVALC were found in this ARD.")
        return

    # Search makes a catalog with hundreds of PARAMCDs practical.
    search = st.text_input("Search variable", placeholder="e.g. MAYO, SESTOT, UCEIS...", key="overview_variable_search")
    choices = catalog
    if search.strip():
        mask = (
    ...
    selected_label = st.selectbox("Variable", choices["Label"].tolist(), key="overview_variable")
    ...
    st.caption(f"Source: {row['Source']}  •  Value type: {row['Value Type']}  •  Column: {row['Column']}")
